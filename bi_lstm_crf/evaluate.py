...
if __name__ == '__main__':
    params = torch.load(
        '/Users/hieunguyen/Desktop/NLP/Master/NER/bi_lstm_crf/vocab_label.pt')
    vocab = params['vocab']
    label = params['label']

    test_dataset = NerDataset(
        '/Users/hieunguyen/Desktop/NLP/Master/NER/data/vlsp2016/test.txt', vocab, label)
    batch_size = 1
    test_iter = DataLoader(test_dataset, batch_size, collate_fn=collate_fn)

    device = torch.device("cpu")
    model = BiLSTM_CRF_NER.load(
        '/Users/hieunguyen/Desktop/NLP/Master/NER/bi_lstm_crf/lstm_ner_6.pt')

    y_test = []
    y_pred = []

    for idx, (sentences, tags) in tqdm(enumerate(test_iter), total=len(test_iter)):
        sentences, sent_lengths = padding(sentences, label.stoi[PAD], device)
